...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'queue_id', 'consumer_id'],
  },
  annotations: {
    idempotentHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  const { consumer_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.queues.consumers.delete(consumer_id, body)),
    );
  } catch (error) {
