...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'index_name', 'propertyName'],
  },
  annotations: {},
};
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.vectorize.indexes.metadataIndex.delete(index_name, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
