...
export const tool: Tool = {
  ...
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nDelete Custom Hostname (and any issued SSL certificates)\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/custom_hostname_delete_response',\n  $defs: {\n    custom_hostname_delete_response: {\n      type: 'object',\n      properties: {\n        id: {\n          type: 'string',\n          description: 'Identifier.'\n        }\n      }\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      zone_id: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  const { custom_hostname_id, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.customHostnames.delete(custom_hostname_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
