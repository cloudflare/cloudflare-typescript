...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.email.security.timeseriesGroups.arc(body)),
    );
  } catch (error) {
    if (isJqError(error)) {
