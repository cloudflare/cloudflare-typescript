...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.attacks.layer3.timeseriesGroups.industry(body)),
    );
  } catch (error) {
    if (isJqError(error)) {
