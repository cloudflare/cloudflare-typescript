  ...
  inputSchema: {
    ...
    properties: {
      dimension: {
        ...
        enum: ['PROTOCOL', 'IP_VERSION', 'VECTOR', 'DURATION', 'BITRATE', 'VERTICAL', 'INDUSTRY'],
      },
      continent: {
        type: 'array',
        description:
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.attacks.layer3.summaryV2(dimension, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
