...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.attacks.layer7.summary.httpVersion(body)),
    );
  } catch (error) {
    if (isJqError(error)) {
