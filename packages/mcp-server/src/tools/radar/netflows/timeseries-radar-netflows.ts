...
export const tool: Tool = {
  name: 'timeseries_radar_netflows',
  ...
  inputSchema: {
    ...
    properties: {
      ...
      product: {
        type: 'array',
        description: 'Array of network traffic product types to filters results by.',
        items: {
          type: 'string',
          enum: ['HTTP', 'ALL'],
        },
...
export const handler = (client: Cloudflare, args: any) => {
  const { ...body } = args;
  return client.radar.netflows.timeseries(body);
};
