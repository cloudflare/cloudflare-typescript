...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  const { http_version, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.http.locations.httpMethod.get(http_version, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
