...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.radar.http.locations.tlsVersion.get(tls_version, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
