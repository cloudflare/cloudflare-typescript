...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['zone_id', 'waiting_room_id', 'event_id'],
  },
  annotations: {
    readOnlyHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.waitingRooms.events.details.get(event_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
