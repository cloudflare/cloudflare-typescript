  ...
  inputSchema: {
    ...
    properties: {
      ...
      queueing_method: {
        ...
        enum: ['fifo', 'random', 'passthrough', 'reject'],
      },
    ...
    required: ['zone_id', 'waiting_room_id', 'host', 'name', 'new_users_per_minute', 'total_active_users'],
    $defs: {
      additional_routes: {
        type: 'object',
        properties: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(await client.waitingRooms.update(waiting_room_id, body));
};
