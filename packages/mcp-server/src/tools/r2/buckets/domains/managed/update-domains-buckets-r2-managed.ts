...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'bucket_name', 'enabled'],
  },
  annotations: {
    idempotentHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.r2.buckets.domains.managed.update(bucket_name, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
