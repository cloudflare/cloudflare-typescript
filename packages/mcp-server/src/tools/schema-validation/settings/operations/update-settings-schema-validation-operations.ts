...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['zone_id', 'operation_id', 'mitigation_action'],
  },
  annotations: {
    idempotentHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.schemaValidation.settings.operations.update(operation_id, body),
      ),
    );
  } catch (error) {
    if (isJqError(error)) {
