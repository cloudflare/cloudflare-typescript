...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'user_id', 'nonce'],
  },
  annotations: {
    readOnlyHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  const { nonce, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.zeroTrust.access.users.activeSessions.get(nonce, body)),
    );
  } catch (error) {
