...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'name', 'upstream_ips'],
    $defs: {
      upstream_ips: {
        type: 'string',
        description: 'Upstream DNS Server IPv4 address',
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.dnsFirewall.create(body)));
  } catch (error) {
