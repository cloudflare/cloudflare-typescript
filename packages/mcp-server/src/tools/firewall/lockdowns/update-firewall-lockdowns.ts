...
export const tool: Tool = {
  name: 'update_firewall_lockdowns',
  description:
  ...
  inputSchema: {
    ...
    required: ['zone_id', 'lock_downs_id', 'configurations', 'urls'],
    $defs: {
      configuration: {
        type: 'array',
        description:
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.firewall.lockdowns.update(lock_downs_id, body)),
  );
};
