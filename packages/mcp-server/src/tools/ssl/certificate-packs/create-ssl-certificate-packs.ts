...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['zone_id', 'certificate_authority', 'hosts', 'type', 'validation_method', 'validity_days'],
    $defs: {
      host: {
        type: 'string',
      },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.ssl.certificatePacks.create(body)));
  } catch (error) {
