...
export const tool: Tool = {
  name: 'get_workers_account_settings',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nFetches Worker account settings for an account.\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/account_setting_get_response',\n  $defs: {\n    account_setting_get_response: {\n      type: 'object',\n      properties: {\n        default_usage_model: {\n          type: 'string'\n        },\n        green_compute: {\n          type: 'boolean'\n        }\n      }\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      account_id: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.workers.accountSettings.get(body)));
  } catch (error) {
    if (isJqError(error)) {
