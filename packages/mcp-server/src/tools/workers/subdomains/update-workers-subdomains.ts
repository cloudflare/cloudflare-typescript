...
export const tool: Tool = {
  ...
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nCreates a Workers subdomain for an account.\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/subdomain_update_response',\n  $defs: {\n    subdomain_update_response: {\n      type: 'object',\n      properties: {\n        subdomain: {\n          type: 'string'\n        }\n      },\n      required: [        'subdomain'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      account_id: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(await maybeFilter(jq_filter, await client.workers.subdomains.update(body)));
  } catch (error) {
    if (isJqError(error)) {
