...
export const tool: Tool = {
  name: 'get_scripts_workers_schedules',
  description:
    "When using this tool, always use the `jq_filter` parameter to reduce the response size and improve performance.\n\nOnly omit if you're sure you don't need the data.\n\nFetches Cron Triggers for a Worker.\n\n# Response Schema\n```json\n{\n  $ref: '#/$defs/schedule_get_response',\n  $defs: {\n    schedule_get_response: {\n      type: 'object',\n      properties: {\n        schedules: {\n          type: 'array',\n          items: {\n            type: 'object',\n            properties: {\n              cron: {\n                type: 'string'\n              },\n              created_on: {\n                type: 'string'\n              },\n              modified_on: {\n                type: 'string'\n              }\n            },\n            required: [              'cron'\n            ]\n          }\n        }\n      },\n      required: [        'schedules'\n      ]\n    }\n  }\n}\n```",
  inputSchema: {
    type: 'object',
    properties: {
      account_id: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.workers.scripts.schedules.get(script_name, body)),
    );
  } catch (error) {
