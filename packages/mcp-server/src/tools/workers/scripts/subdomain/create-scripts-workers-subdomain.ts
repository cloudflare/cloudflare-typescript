...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'script_name', 'enabled'],
  },
  annotations: {},
};
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.workers.scripts.subdomain.create(script_name, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
