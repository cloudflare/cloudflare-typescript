...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'address_map_id', 'ip_address', 'body'],
  },
  annotations: {
    idempotentHint: true,
  },
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  const { ip_address, jq_filter, ...body } = args as any;
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.addressing.addressMaps.ips.update(ip_address, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
