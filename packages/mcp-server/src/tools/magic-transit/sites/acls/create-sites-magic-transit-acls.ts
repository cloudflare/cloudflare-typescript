...
export const tool: Tool = {
  name: 'create_sites_magic_transit_acls',
  description:
  ...
  inputSchema: {
    ...
    required: ['account_id', 'site_id', 'lan_1', 'lan_2', 'name'],
    $defs: {
      acl_configuration: {
        type: 'object',
        properties: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  return asTextContentResult(
    await maybeFilter(jq_filter, await client.magicTransit.sites.acls.create(site_id, body)),
  );
};
