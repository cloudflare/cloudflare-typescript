...
export const tool: Tool = {
  ...
  inputSchema: {
    ...
    required: ['account_id', 'namespace_id', 'keys'],
  },
  annotations: {},
};
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.kv.namespaces.bulkGet(namespace_id, body)),
    );
  } catch (error) {
    if (isJqError(error)) {
