...
export const tool: Tool = {
  name: 'update_tables_namespaces_r2_data_catalog_maintenance_configs',
  description:
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(
        jq_filter,
        await client.r2DataCatalog.namespaces.tables.maintenanceConfigs.update(table_name, body),
      ),
    );
  } catch (error) {
