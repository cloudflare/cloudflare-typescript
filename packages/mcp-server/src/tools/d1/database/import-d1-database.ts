    ...
    anyOf: [
      {
        ...
        required: ['account_id', 'database_id', 'action', 'etag'],
      },
      {
        type: 'object',
        properties: {
        ...
        required: ['account_id', 'database_id', 'action', 'etag', 'filename'],
      },
      {
        type: 'object',
        properties: {
        ...
        required: ['account_id', 'database_id', 'action', 'current_bookmark'],
      },
    ],
    properties: {
      jq_filter: {
...
export const handler = async (client: Cloudflare, args: Record<string, unknown> | undefined) => {
  ...
  try {
    return asTextContentResult(
      await maybeFilter(jq_filter, await client.d1.database.import(database_id, body)),
    );
  } catch (error) {
