...
export class Regions extends APIResource {
  ...
  list(params: RegionListParams, options?: Core.RequestOptions): Core.APIPromise<RegionListResponse> {
    ...
    return (this._client.get(`/accounts/${account_id}/load_balancers/regions`, { query, ...options }) as Core.APIPromise<{ result: RegionListResponse }>)._thenUnwrap((obj) => obj.result);
  }
  ...
  get(regionId: 'WNAM' | 'ENAM' | 'WEU' | 'EEU' | 'NSAM' | 'SSAM' | 'OC' | 'ME' | 'NAF' | 'SAF' | 'SAS' | 'SEAS' | 'NEAS', params: RegionGetParams, options?: Core.RequestOptions): Core.APIPromise<RegionGetResponse> {
    ...
    return (this._client.get(`/accounts/${account_id}/load_balancers/regions/${regionId}`, options) as Core.APIPromise<{ result: RegionGetResponse }>)._thenUnwrap((obj) => obj.result);
  }
}
