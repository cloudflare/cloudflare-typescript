...
export class WebCrawlers extends APIResource {
  ...
  summary(
    ...
    return (
      this._client.get(path`/radar/bots/crawlers/summary/${dimension}`, { query, ...options }) as APIPromise<{
        result: WebCrawlerSummaryResponse;
      }>
  ...
  timeseriesGroups(
    ...
    return (
      this._client.get(path`/radar/bots/crawlers/timeseries_groups/${dimension}`, {
        query,
        ...options,
      }) as APIPromise<{ result: WebCrawlerTimeseriesGroupsResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
}
