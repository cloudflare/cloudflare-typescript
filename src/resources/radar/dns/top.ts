...
export class BaseTop extends APIResource {
  ...
  ases(query: TopAsesParams | null | undefined = {}, options?: RequestOptions): APIPromise<TopAsesResponse> {
    return (
      this._client.get('/radar/dns/top/ases', { query, ...options }) as APIPromise<{
  ...
  locations(
    ...
    return (
      this._client.get('/radar/dns/top/locations', { query, ...options }) as APIPromise<{
...
export interface TopAsesParams {
  ...
  /**
   * Filters results by domain name. When set, no other DNS filter may be used — only
