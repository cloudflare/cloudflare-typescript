...
export class BaseURLIgnorePatterns extends APIResource {
  static override readonly _key: readonly ['emailSecurity', 'settings', 'urlIgnorePatterns'] = Object.freeze([
    'emailSecurity',
    'settings',
    'urlIgnorePatterns',
  ] as const);
  ...
  ): APIPromise<URLIgnorePatternCreateResponse> {
    const { account_id, ...body } = params;
    return (
      this._client.post(path`/accounts/${account_id}/email-security/settings/url_ignore_patterns`, {
        body,
        ...options,
      }) as APIPromise<{ result: URLIgnorePatternCreateResponse }>
  ...
  list(
    ...
    options?: RequestOptions,
  ): PagePromise<URLIgnorePatternListResponsesV4PagePaginationArray, URLIgnorePatternListResponse> {
  ...
  delete(
    ...
    options?: RequestOptions,
  ): APIPromise<URLIgnorePatternDeleteResponse> {
    const { account_id } = params;
    return (
      this._client.delete(
        path`/accounts/${account_id}/email-security/settings/url_ignore_patterns/${patternID}`,
        options,
      ) as APIPromise<{ result: URLIgnorePatternDeleteResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
  ...
  edit(
    ...
    options?: RequestOptions,
  ): APIPromise<URLIgnorePatternEditResponse> {
    const { account_id, ...body } = params;
    return (
      this._client.patch(
        path`/accounts/${account_id}/email-security/settings/url_ignore_patterns/${patternID}`,
        { body, ...options },
      ) as APIPromise<{ result: URLIgnorePatternEditResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
  ...
  get(
    ...
    options?: RequestOptions,
  ): APIPromise<URLIgnorePatternGetResponse> {
    const { account_id } = params;
    return (
      this._client.get(
        path`/accounts/${account_id}/email-security/settings/url_ignore_patterns/${patternID}`,
        options,
      ) as APIPromise<{ result: URLIgnorePatternGetResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
}
export class URLIgnorePatterns extends BaseURLIgnorePatterns {}

export type URLIgnorePatternListResponsesV4PagePaginationArray =
  V4PagePaginationArray<URLIgnorePatternListResponse>;
