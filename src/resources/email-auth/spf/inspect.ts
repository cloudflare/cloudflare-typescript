...
export class BaseInspect extends APIResource {
  static override readonly _key: readonly ['emailAuth', 'spf', 'inspect'] = Object.freeze([
    'emailAuth',
    'spf',
    'inspect',
  ] as const);
   ...
   * ```
   */
  get(params: InspectGetParams, options?: RequestOptions): APIPromise<InspectGetResponse> {
    const { zone_id, ...query } = params;
    return (
      this._client.get(path`/zones/${zone_id}/email/auth/spf/inspect`, { query, ...options }) as APIPromise<{
        result: InspectGetResponse;
      }>
    )._thenUnwrap((obj) => obj.result);
  }
}
export class Inspect extends BaseInspect {}
