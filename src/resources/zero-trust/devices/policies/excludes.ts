...
export class Excludes extends APIResource {
  ...
  update(params: ExcludeUpdateParams, options?: Core.RequestOptions): Core.APIPromise<ExcludeUpdateResponse | null> {
    ...
    return (this._client.put(`/accounts/${account_id}/devices/policy/exclude`, { body: body, ...options }) as Core.APIPromise<{ result: ExcludeUpdateResponse | null }>)._thenUnwrap((obj) => obj.result);
  }
  ...
  get(policyId: string, params: ExcludeGetParams, options?: Core.RequestOptions): Core.APIPromise<ExcludeGetResponse | null> {
    const { account_id } = params;
    return (this._client.get(`/accounts/${account_id}/devices/policy/${policyId}/exclude`, options) as Core.APIPromise<{ result: ExcludeGetResponse | null }>)._thenUnwrap((obj) => obj.result);
  }
}
