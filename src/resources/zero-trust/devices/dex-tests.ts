...
export class DEXTests extends APIResource {
   ...
   * Create a DEX test.
   */
  create(params: DEXTestCreateParams, options?: RequestOptions): APIPromise<SchemaHTTP | null> {
    const { account_id, ...body } = params;
    return (
      this._client.post(path`/accounts/${account_id}/devices/dex_tests`, { body, ...options }) as APIPromise<{
  ...
  ): APIPromise<SchemaHTTP | null> {
    const { account_id, ...body } = params;
    return (
      this._client.put(path`/accounts/${account_id}/devices/dex_tests/${dexTestID}`, {
        body,
        ...options,
      }) as APIPromise<{ result: SchemaHTTP | null }>
    )._thenUnwrap((obj) => obj.result);
  }
   ...
   * Fetch all DEX tests.
   */
  list(params: DEXTestListParams, options?: RequestOptions): PagePromise<SchemaHTTPSSinglePage, SchemaHTTP> {
    const { account_id } = params;
    return this._client.getAPIList(
      path`/accounts/${account_id}/devices/dex_tests`,
      SinglePage<SchemaHTTP>,
      options,
    );
  }
  ...
  delete(
    ...
    return (
      this._client.delete(
        path`/accounts/${account_id}/devices/dex_tests/${dexTestID}`,
        options,
   ...
   */
  get(dexTestID: string, params: DEXTestGetParams, options?: RequestOptions): APIPromise<SchemaHTTP | null> {
    const { account_id } = params;
    return (
      this._client.get(path`/accounts/${account_id}/devices/dex_tests/${dexTestID}`, options) as APIPromise<{
        result: SchemaHTTP | null;
      }>
    )._thenUnwrap((obj) => obj.result);
  }
...
export type SchemaHTTPSSinglePage = SinglePage<SchemaHTTP>;
...
export declare namespace DEXTests {
  export {
    ...
    type SchemaHTTPSSinglePage as SchemaHTTPSSinglePage,
