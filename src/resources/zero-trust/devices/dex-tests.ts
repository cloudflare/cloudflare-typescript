...
export class DEXTests extends APIResource {
  ...
  list(
    ...
    options?: RequestOptions,
  ): PagePromise<DEXTestListResponsesSinglePage, DEXTestListResponse> {
    const { account_id } = params;
    return this._client.getAPIList(
      path`/accounts/${account_id}/devices/dex_tests`,
      SinglePage<DEXTestListResponse>,
      options,
    );
  }
  ...
  get(dexTestID: string, params: DEXTestGetParams, options?: RequestOptions): APIPromise<DEXTestGetResponse> {
    ...
    return (
      this._client.get(path`/accounts/${account_id}/devices/dex_tests/${dexTestID}`, options) as APIPromise<{
        result: DEXTestGetResponse;
      }>
    )._thenUnwrap((obj) => obj.result);
  }
}

export type DEXTestListResponsesSinglePage = SinglePage<DEXTestListResponse>;
