  ...
  delete(
    ...
    return (
      this._client.delete(
        ...
        options,
      ) as APIPromise<{ result: CustomCertificateDeleteResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
...
export type CustomCertificatesSinglePage = SinglePage<CustomCertificate>;
...
export interface CustomCertificateCreateParams {
  ...
  /**
   * Body param: The zone's private key. Not required if custom_csr_id is provided,
   * in which case the private key is retrieved from the CSR record held by
   * Cloudflare.
   */
  private_key?: string;
