...
export class BaseTopup extends APIResource {
  static override readonly _key: readonly ['aiGateway', 'billing', 'topup'] = Object.freeze([
    'aiGateway',
    'billing',
    'topup',
  ] as const);
   ...
   * ```
   */
  create(params: TopupCreateParams, options?: RequestOptions): APIPromise<TopupCreateResponse> {
    const { account_id, ...body } = params;
    return (
      this._client.post(path`/accounts/${account_id}/ai-gateway/billing/topup`, {
        body,
        ...options,
      }) as APIPromise<{ result: TopupCreateResponse }>
   ...
   * ```
   */
  status(params: TopupStatusParams, options?: RequestOptions): APIPromise<TopupStatusResponse> {
    const { account_id, ...body } = params;
    return (
      this._client.post(path`/accounts/${account_id}/ai-gateway/billing/topup/status`, {
        body,
        ...options,
      }) as APIPromise<{ result: TopupStatusResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
}
export class Topup extends BaseTopup {
