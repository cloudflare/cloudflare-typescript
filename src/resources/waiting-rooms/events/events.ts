...
export class Events extends APIResource {
  ...
  replace(
    ...
    waitingRoomId: unknown,
    eventId: unknown,
    body: EventReplaceParams,
    options?: Core.RequestOptions,
  ): Core.APIPromise<EventReplaceResponse> {
    return (
      this._client.put(`/zones/${zoneIdentifier}/waiting_rooms/${waitingRoomId}/events/${eventId}`, {
        body,
        ...options,
      }) as Core.APIPromise<{ result: EventReplaceResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
}
...
export interface EventReplaceResponse {
  id?: unknown;
...
export interface EventReplaceParams {
  /**
   * An ISO 8601 timestamp that marks the end of the event.
   */
  event_end_time: string;
