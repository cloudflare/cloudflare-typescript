...
export class ASN extends APIResource {
  ...
  delete(asnId: number, params: ASNDeleteParams, options?: Core.RequestOptions): Core.APIPromise<ASNDeleteResponse> {
    ...
    return (this._client.delete(`/accounts/${account_id}/botnet_feed/configs/asn/${asnId}`, options) as Core.APIPromise<{ result: ASNDeleteResponse }>)._thenUnwrap((obj) => obj.result);
  }
  ...
  get(params: ASNGetParams, options?: Core.RequestOptions): Core.APIPromise<ASNGetResponse> {
    const { account_id } = params;
    return (this._client.get(`/accounts/${account_id}/botnet_feed/configs/asn`, options) as Core.APIPromise<{ result: ASNGetResponse }>)._thenUnwrap((obj) => obj.result);
  }
}
