...
export class ASN extends APIResource {
  ...
  dayReport(asnId: number, params: ASNDayReportParams, options?: Core.RequestOptions): Core.APIPromise<ASNDayReportResponse> {
    ...
    return (this._client.get(`/accounts/${account_id}/botnet_feed/asn/${asnId}/day_report`, { query, ...options }) as Core.APIPromise<{ result: ASNDayReportResponse }>)._thenUnwrap((obj) => obj.result);
  }
  ...
  fullReport(asnId: number, params: ASNFullReportParams, options?: Core.RequestOptions): Core.APIPromise<ASNFullReportResponse> {
    ...
    return (this._client.get(`/accounts/${account_id}/botnet_feed/asn/${asnId}/full_report`, options) as Core.APIPromise<{ result: ASNFullReportResponse }>)._thenUnwrap((obj) => obj.result);
  }
}
