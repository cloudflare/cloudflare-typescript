...
export class BaseAutoOriginTLSKex extends APIResource {
  ...
  ): APIPromise<AutoOriginTLSKexEditResponse> {
    const { zone_id, ...body } = params;
    return (
      this._client.patch(path`/zones/${zone_id}/settings/auto_origin_tls_kex`, {
        body,
        ...options,
      }) as APIPromise<{ result: AutoOriginTLSKexEditResponse }>
   ...
   * ```
   */
  get(params: AutoOriginTLSKexGetParams, options?: RequestOptions): APIPromise<AutoOriginTLSKexGetResponse> {
    const { zone_id } = params;
    return (
      this._client.get(path`/zones/${zone_id}/settings/auto_origin_tls_kex`, options) as APIPromise<{
