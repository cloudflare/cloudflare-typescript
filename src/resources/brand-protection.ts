...
export class BrandProtection extends APIResource {
   ...
   */
  submit(params: BrandProtectionSubmitParams, options?: RequestOptions): APIPromise<Submit> {
    const { account_id, ...body } = params;
    return (
      this._client.post(path`/accounts/${account_id}/brand-protection/submit`, {
        body,
        ...options,
      }) as APIPromise<{ result: Submit }>
   ...
   */
  urlInfo(params: BrandProtectionURLInfoParams, options?: RequestOptions): APIPromise<Info> {
    const { account_id, ...query } = params;
    return (
      this._client.get(path`/accounts/${account_id}/brand-protection/url-info`, {
        query,
        ...options,
      }) as APIPromise<{ result: Info }>
    )._thenUnwrap((obj) => obj.result);
  }
}
