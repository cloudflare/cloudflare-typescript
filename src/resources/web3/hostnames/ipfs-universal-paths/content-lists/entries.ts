...
export class Entries extends APIResource {
  ...
  replace(
    ...
    identifier: string,
    contentListEntryIdentifier: string,
    body: EntryReplaceParams,
    options?: Core.RequestOptions,
  ): Core.APIPromise<EntryReplaceResponse> {
    return (
      this._client.put(
        `/zones/${zoneIdentifier}/web3/hostnames/${identifier}/ipfs_universal_path/content_list/entries/${contentListEntryIdentifier}`,
        { body, ...options },
      ) as Core.APIPromise<{ result: EntryReplaceResponse }>
    )._thenUnwrap((obj) => obj.result);
  }
}
 ...
 * Content list entry to be blocked.
 */
export interface EntryReplaceResponse {
  /**
   * Identifier
   */
  id?: string;
...
export interface EntryReplaceParams {
  /**
   * CID or content path of content to block.
   */
  content: string;
