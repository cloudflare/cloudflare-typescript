...
import { BaseKeys } from 'cloudflare/resources/resource-tagging/keys';
...
const runTests = (client: PartialCloudflare<{ resourceTagging: { keys: BaseKeys } }>) => {
  test('list: only required params', async () => {
    const responsePromise = client.resourceTagging.keys.list({
  ...
  test('list: required and optional params', async () => {
    const response = await client.resourceTagging.keys.list({
      account_id: '023e105f4ecef8ad9ca31a8372d0c353',
