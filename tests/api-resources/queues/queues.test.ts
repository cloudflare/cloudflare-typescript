...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseQueues],
});

const runTests = (client: PartialCloudflare<{ queues: BaseQueues }>) => {
  ...
  test('getMetrics: only required params', async () => {
    const responsePromise = client.queues.getMetrics('023e105f4ecef8ad9ca31a8372d0c353', {
      account_id: '023e105f4ecef8ad9ca31a8372d0c353',
    });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
...
describe('resource queues', () => runTests(client));
describe('resource queues (tree shakable, base)', () => runTests(partialClient));
