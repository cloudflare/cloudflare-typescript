...
import { BaseLoadBalancers } from 'cloudflare/resources/load-balancers/load-balancers';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseLoadBalancers],
});

const runTests = (client: PartialCloudflare<{ loadBalancers: BaseLoadBalancers }>) => {
  ...
  test('list', async () => {
    const responsePromise = client.loadBalancers.list({ account_id: 'account_id' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
...
describe('resource loadBalancers', () => runTests(client));
describe('resource loadBalancers (tree shakable, base)', () => runTests(partialClient));
