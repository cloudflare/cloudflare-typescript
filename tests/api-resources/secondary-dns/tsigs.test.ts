...
describe('resource tsigs', () => {
  ...
  test.skip('secondaryDNSTsigCreateTsig: only required params', async () => {
    const responsePromise = cloudflare.secondaryDNS.tsigs.secondaryDNSTsigCreateTsig(
      '01a7362d577a6c3019a474fd6f485823',
      {
        algo: 'hmac-sha512.',
        name: 'tsig.customer.cf.',
  ...
  test.skip('secondaryDNSTsigCreateTsig: required and optional params', async () => {
    const response = await cloudflare.secondaryDNS.tsigs.secondaryDNSTsigCreateTsig(
      '01a7362d577a6c3019a474fd6f485823',
      {
        algo: 'hmac-sha512.',
        name: 'tsig.customer.cf.',
  ...
  test.skip('secondaryDNSTsigListTsiGs', async () => {
    const responsePromise = cloudflare.secondaryDNS.tsigs.secondaryDNSTsigListTsiGs(
      '01a7362d577a6c3019a474fd6f485823',
    );
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
  ...
  test.skip('secondaryDNSTsigListTsiGs: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      cloudflare.secondaryDNS.tsigs.secondaryDNSTsigListTsiGs('01a7362d577a6c3019a474fd6f485823', {
        path: '/_stainless_unknown_path',
      }),
    ).rejects.toThrow(Cloudflare.NotFoundError);
  });
