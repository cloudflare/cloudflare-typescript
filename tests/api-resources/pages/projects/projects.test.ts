...
const runTests = (client: PartialCloudflare<{ pages: { projects: BaseProjects } }>) => {
  test('create: only required params', async () => {
    const responsePromise = client.pages.projects.create({
    ...
    const response = await client.pages.projects.create({
      ...
      deployment_configs: {
        preview: {
          ...
          env_vars: { foo: { type: 'plain_text', value: 'hello world' } },
          fail_open: true,
...
describe('resource projects (tree shakable, subresource)', () => runTests(parentPartialClient));
