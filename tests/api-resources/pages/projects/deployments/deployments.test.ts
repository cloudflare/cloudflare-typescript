...
const runTests = (client: PartialCloudflare<{ pages: { projects: { deployments: BaseDeployments } } }>) => {
  ...
  test.skip('create: required and optional params', async () => {
    const response = await client.pages.projects.deployments.create('this-is-my-project-01', {
      account_id: '023e105f4ecef8ad9ca31a8372d0c353',
      _headers: await toFile(Buffer.from('Example data'), 'README.md'),
      _redirects: await toFile(Buffer.from('Example data'), 'README.md'),
      '_routes.json': await toFile(Buffer.from('Example data'), 'README.md'),
      '_worker.bundle': await toFile(Buffer.from('Example data'), 'README.md'),
      '_worker.js': await toFile(Buffer.from('Example data'), 'README.md'),
      branch: 'staging',
      commit_dirty: 'false',
      commit_hash: 'a1b2c3d4e5f6',
      commit_message: 'Update homepage',
      'functions-filepath-routing-config.json': await toFile(Buffer.from('Example data'), 'README.md'),
