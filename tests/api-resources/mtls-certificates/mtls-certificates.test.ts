...
import { BaseMTLSCertificates } from 'cloudflare/resources/mtls-certificates/mtls-certificates';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseMTLSCertificates],
});

const runTests = (client: PartialCloudflare<{ mtlsCertificates: BaseMTLSCertificates }>) => {
...
describe('resource mtlsCertificates', () => runTests(client));
describe('resource mtlsCertificates (tree shakable, base)', () => runTests(partialClient));
