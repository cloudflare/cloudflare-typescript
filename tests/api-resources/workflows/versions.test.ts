...
describe('resource versions', () => {
  ...
  test('graph: only required params', async () => {
    const responsePromise = client.workflows.versions.graph('x', '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
      account_id: 'account_id',
    });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
  ...
  test('graph: required and optional params', async () => {
    const response = await client.workflows.versions.graph('x', '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e', {
      account_id: 'account_id',
    });
  });
});
