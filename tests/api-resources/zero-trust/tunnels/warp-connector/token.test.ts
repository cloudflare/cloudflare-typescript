...
import { BaseToken } from 'cloudflare/resources/zero-trust/tunnels/warp-connector/token';
...
const runTests = (
  client: PartialCloudflare<{ zeroTrust: { tunnels: { warpConnector: { token: BaseToken } } } }>,
) => {
  test('get: only required params', async () => {
    const responsePromise = client.zeroTrust.tunnels.warpConnector.token.get(
  ...
  test('get: required and optional params', async () => {
    const response = await client.zeroTrust.tunnels.warpConnector.token.get(
      'f70ff985-a4ef-4643-bbbc-4a0ed4fc8415',
      { account_id: '699d98642c564d2e855e9661899b7252' },
    );
  });
