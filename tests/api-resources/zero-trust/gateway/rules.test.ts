...
import { BaseRules } from 'cloudflare/resources/zero-trust/gateway/rules';
...
const runTests = (client: PartialCloudflare<{ zeroTrust: { gateway: { rules: BaseRules } } }>) => {
  ...
  test('create: required and optional params', async () => {
    const response = await client.zeroTrust.gateway.rules.create({
      ...
      enabled: true,
      expiration: { expires_at: '2014-01-01T05:20:20Z', duration: 10 },
      filters: ['http'],
      ...
      rule_settings: {
        ...
        block_page: { target_uri: 'https://example.com', include_context: true },
