...
import { BaseResourceSharing } from 'cloudflare/resources/resource-sharing/resource-sharing';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseResourceSharing],
});

const runTests = (client: PartialCloudflare<{ resourceSharing: BaseResourceSharing }>) => {
...
describe('resource resourceSharing', () => runTests(client));
describe('resource resourceSharing (tree shakable, base)', () => runTests(partialClient));
