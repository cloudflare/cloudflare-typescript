...
import { BaseOwnershipResource } from 'cloudflare/resources/magic-transit/pcaps/ownership';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseOwnershipResource],
});
...
const runTests = (
  client: PartialCloudflare<{ magicTransit: { pcaps: { ownership: BaseOwnershipResource } } }>,
) => {
...
describe('resource ownership', () => runTests(client));
describe('resource ownership (tree shakable, base)', () => runTests(partialClient));
describe('resource ownership (tree shakable, subresource)', () => runTests(parentPartialClient));
