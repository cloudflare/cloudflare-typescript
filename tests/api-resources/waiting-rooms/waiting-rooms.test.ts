...
import { BaseWaitingRooms } from 'cloudflare/resources/waiting-rooms/waiting-rooms';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseWaitingRooms],
});

const runTests = (client: PartialCloudflare<{ waitingRooms: BaseWaitingRooms }>) => {
...
describe('resource waitingRooms', () => runTests(client));
describe('resource waitingRooms (tree shakable, base)', () => runTests(partialClient));
