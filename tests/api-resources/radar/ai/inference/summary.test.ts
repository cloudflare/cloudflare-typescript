...
import { Inference } from 'cloudflare/resources/radar/ai/inference/inference';
import { BaseSummary } from 'cloudflare/resources/radar/ai/inference/summary';
...
const runTests = (client: PartialCloudflare<{ radar: { ai: { inference: { summary: BaseSummary } } } }>) => {
  test('model', async () => {
    const responsePromise = client.radar.ai.inference.summary.model();
  ...
  test('task', async () => {
    const responsePromise = client.radar.ai.inference.summary.task();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
