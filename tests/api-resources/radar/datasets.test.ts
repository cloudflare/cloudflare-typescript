...
describe('resource datasets', () => {
  ...
  test('list: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.radar.datasets.list({ path: '/_stainless_unknown_path' }))
      .rejects
  ...
  test('list: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.radar.datasets.list({ datasetType: 'RANKING_BUCKET', format: 'JSON', limit: 5, offset: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Cloudflare.NotFoundError);
  });
  ...
  test('get: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.radar.datasets.get('ranking_top_1000', { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(Cloudflare.NotFoundError);
  });
});
