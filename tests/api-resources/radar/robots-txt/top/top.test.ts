...
describe('resource top', () => {
  ...
  test('domainCategories: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.radar.robotsTXT.top.domainCategories({ path: '/_stainless_unknown_path' }),
  ...
  test('domainCategories: request options and params are passed correctly', async () => {
    ...
    await expect(
      client.radar.robotsTXT.top.domainCategories(
        { date: ['2019-12-27'], format: 'JSON', limit: 5, name: ['string'], userAgentCategory: 'AI' },
