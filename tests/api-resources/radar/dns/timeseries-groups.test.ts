...
import { BaseTimeseriesGroups } from 'cloudflare/resources/radar/dns/timeseries-groups';
...
const runTests = (
  client: PartialCloudflare<{ radar: { dns: { timeseriesGroups: BaseTimeseriesGroups } } }>,
) => {
  test('cacheHit', async () => {
  ...
  test('responseCode', async () => {
    const responsePromise = client.radar.dns.timeseriesGroups.responseCode();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
