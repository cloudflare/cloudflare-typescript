...
describe('resource summary', () => {
  ...
  test('botClass: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.radar.leakedCredentials.summary.botClass({ path: '/_stainless_unknown_path' }),
  ...
  test('compromised: request options instead of params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.radar.leakedCredentials.summary.compromised({ path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Cloudflare.NotFoundError);
  });
