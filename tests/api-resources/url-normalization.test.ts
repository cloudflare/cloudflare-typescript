...
import { BaseURLNormalization } from 'cloudflare/resources/url-normalization/url-normalization';
...
const partialClient = createClient({
  ...
  apiEmail: 'user@example.com',
  baseURL: process.env['TEST_API_BASE_URL'] ?? 'http://127.0.0.1:4010',
  resources: [BaseURLNormalization],
});

const runTests = (client: PartialCloudflare<{ urlNormalization: BaseURLNormalization }>) => {
...
describe('resource urlNormalization', () => runTests(client));
describe('resource urlNormalization (tree shakable, base)', () => runTests(partialClient));
