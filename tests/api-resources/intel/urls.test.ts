...
import { BaseURLs } from 'cloudflare/resources/intel/urls';
...
const runTests = (client: PartialCloudflare<{ intel: { urls: BaseURLs } }>) => {
  test('get: only required params', async () => {
    const responsePromise = client.intel.urls.get({
  ...
  test('get: required and optional params', async () => {
    const response = await client.intel.urls.get({
      account_id: '023e105f4ecef8ad9ca31a8372d0c353',
      url: 'url',
    });
  });
