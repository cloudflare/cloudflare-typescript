...
describe('resource snapshot', () => {
  test('create', async () => {
    const responsePromise = client.browsingRendering.snapshot.create('accountId');
  ...
  test('create: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      client.browsingRendering.snapshot.create('accountId', { path: '/_stainless_unknown_path' }),
  ...
  test('create: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.browsingRendering.snapshot.create(
        'accountId',
        {
          cacheTTL: 86400,
